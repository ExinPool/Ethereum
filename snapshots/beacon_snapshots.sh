...
do
    ...
    balanceBig=`curl --silent -X 'GET' 'https://beaconcha.in/api/v1/validator/$node?apikey=$api_key' -H 'accept: application/json' | jq . | grep -w balance | awk -F':' '{print $2}' | sed "s/ //g" | sed "s/,//g"`
